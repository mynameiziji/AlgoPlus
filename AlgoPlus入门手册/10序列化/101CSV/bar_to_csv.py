...
class TickEngine(MdApi):
    def __init__(self, md_server, broker_id, investor_id, password, app_id, auth_code
        ...
        self.csv_file_dict = {}  # file object 字典
        self.csv_writer_dict = {}  # writer object 字典

        header = list(bar_field)  # csv表头
        # 遍历订阅列表
        for instrument_id in self.instrument_id_list:
            ...
            # file object
            self.csv_file_dict[instrument_id] = open(f"{instrument_id.decode('utf-8')}-{self.GetTradingDay().decode('utf-8')}-bar.csv", 'w', newline='')
            # writer object
            self.csv_writer_dict[instrument_id] = csv.DictWriter(self.csv_file_dict[instrument_id], header)
            # 写入表头
            self.csv_writer_dict[instrument_id].writeheader()
    ...
    def OnRtnDepthMarketData(self, pDepthMarketData):
        ...
        if is_new_bar:
            ...
            self.csv_writer_dict[pDepthMarketData.InstrumentID].writerow(self.bar_field_dict[pDepthMarketData.InstrumentID])
